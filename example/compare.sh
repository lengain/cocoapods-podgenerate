...
#    3. 重装 cocoapods-podgenerate gem（从本地源码构建）
...
install_podgen_gem() {
  local gemfile="$PODGEN_DIR/cocoapods-podgenerate-0.1.11.gem"
  if [ ! -f "$gemfile" ]; then
    echo "  构建 cocoapods-podgenerate 0.1.11 gem..."
    cd "$PODGEN_DIR"
    gem build cocoapods-podgenerate.gemspec -o "$(basename "$gemfile")" 2>/dev/null
    cd "$BASE_DIR"
  fi
  echo "  安装 cocoapods-podgenerate 0.1.11 gem..."
